    ...
    for index, image in enumerate(images):
        ...
        for mask_index, mask in enumerate(masks):
            ...
            pixels[image_mask > 128] = pixels[image_mask > 128] * (1 - mask_alpha) + \
                                       (mask_alpha * np.array(get_colors(mask_index, max_value)).astype(
                                           np.float)).astype(np.uint8)
...
def parse_args():
    ...
    parser.add_argument("-r", "--inference_results",
                        help="Results of your inference model. Can be a path to a file or numpy array")
